...
impl TokenPair<HumanAddr> {
    ...
    pub fn query_balances(
        ...
        let token0_amount = self.0.query_balance(querier, exchange_addr.clone(), viewing_key.clone())?;
        let token1_amount = self.1.query_balance(querier, exchange_addr.clone(), viewing_key.clone())?;
        // order is important
